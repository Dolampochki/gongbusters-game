$(document).ready(function() {
	...
	$(".gong-button").click(function(){
		clearTimeout(timer);
		$(".cards .game-card-" + (currentCard + 1)).addClass(" animated rotateOutDownRight");
		$(".cards .game-card-" + (currentCard + 1) + " .card-video").trigger('pause');	
		$(".cards .game-card-" + (currentCard + 2) + " .card-video").trigger('play');
	    $(".cards .game-card-" + (currentCard + 2) + " .card-performance-timer").html("<img src='img/timer_white.gif?timestamp=" + new Date().getTime() + "' />");
		timer = null;
